from fastapi import FastAPI ,Path,HTTPException,Query # fastapi framework
import json
app = FastAPI()
...
def sort_patients(sort_by:str=Query(..., description = 'Sort on the basis of height,weight or bmi')
  ...
  if sort_by not in valid_fields: 
    ...
    if order not in ['asc','desc']:
        raise HTTPException(status_code=400, detail="Invalid order select between from asc or desc")
